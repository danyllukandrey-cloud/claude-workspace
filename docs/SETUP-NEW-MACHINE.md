...
Перевірки, що спрацьовують під час коміту, лежать у папці `.githooks/`. Сама папка
приходить із репозиторієм, але Git треба **один раз** показати на неї:
...
> **Чому це окремий крок.** Стандартна папка `.git/hooks/` лежить поза репозиторієм
> і через git не передається. Без цієї команди хуки на новій машині мовчки не працюють —
> а мовчазна відмова найнеприємніша: здається, що все гаразд.
