...
### US-17: Видалити блок-метрику, який більше не потрібен
**As a** user
**I want** прибрати окремий блок-метрику з картки, коли я більше не відстежую цей показник
**So that** картка показує лише ті метрики, які я справді веду зараз

### US-18: Тримати картку станом-індикатором, без жодних вимірювань ([CH-02](changes.md))
**As a** user
**I want** позначити картку одним із трьох станів (використовується / критично потребує відновлення / на паузі) кольоровим індикатором замість вимірюваних метрик
**So that** я можу відстежувати зони життя, які не піддаються кількісному виміру, тим самим кольором-сигналом, що й решта продукту
...
**Then** система повертає `card.status` в `active`; позиція в розкладці Структури НЕ відновлюється автоматично на стару клітинку (D-69) — картка з'являється внизу екрана розкладки без призначеної клітинки, і користувач перетягує її на потрібне місце сам ([D-104](../../DECISIONS.md#d-104); той самий UI-патерн, що вже описаний для зміни режиму розкладки, `structure/spec.md` AC-11b)
...
### AC-20 (US-17) — happy path
**Given** користувач відкрив картку з блоком-метрикою, яку більше не хоче відстежувати
**When** користувач тисне хрестик на блоці й підтверджує видалення, ввівши слово «видалити» (Enter або натиснувши галочку)
**Then** система позначає блок-метрику архівованою (`status: archived`, [D-127](../../DECISIONS.md#d-127), ніколи фізичне видалення) — блок одразу зникає зі списку блоків картки, а його історія записів лишається технічно збереженою

### AC-21 (US-18) — happy path
**Given** користувач створює чи редагує картку
**When** користувач обирає режим «Картка: стан без вимірювань» (пункт над «Картка: постійний процес з метриками (без дати)»)
**Then** усі інші налаштування картки (блоки-метрики, цілі, частота) стають неактивними — прогрес картки більше не рахується з жодного числа, лише зі стану

### AC-22 (US-18) — три стани, крос-контекстний показ
**Given** картка має режим «стан без вимірювань»
**When** користувач обирає один із трьох станів — використовується (зелений) / критично потребує відновлення (червоний) / на паузі (жовтий)
**Then** система показує колір м'ячиком у правому верхньому кутку картки (той самий патерн, що позначка зміни запису) — на самій картці, в колоді і, координовано з `structure` CH-02, на Схемі й Аналітиці (`structure/spec.md` AC-19)
...
| Офлайн-доступність (читання) | 100% — картка й історія відкриваються з кешу без мережі, за умови що кеш уже наповнений (виняток — перше відкриття картки на новому пристрої, D-106: холодний старт вимагає мережі рівно один раз, sad.md Critical flow 4) | ручна перевірка без з'єднання (на пристрої, де картка вже відкривалась раніше) |
...
- [x] ~~Синхронізація архівації з розкладкою Структури (AC-16, [D-66](../../DECISIONS.md#d-66)) — коли картку архівують тут, позиція в `structure_layout_position` не закривається сама собою.~~ Закрито 2026-08-24 ([D-69](../../DECISIONS.md#d-69)) — той самий бекенд-запит, що архівує картку, у тій самій транзакції закриває її активну позицію в `structure_layout_position` (обидві таблиці в одній базі, D-59, окремий зв'язок/черга не потрібні). **Уточнення 2026-09-05:** рішення D-69 було записане 2026-08-24, але код цього не робив аж до [D-103](../../DECISIONS.md#d-103) — таблиця `structure_layout_position` взагалі не існувала в реальній базі ([ISS-26](../../ISSUES.md)). Реально запрацювало й перевірено проти Neon лише 2026-09-05.
...
- [ ] **AC-08: стан `in_use` («ведеться») ніколи не записується жодним реальним викликом** (`/sdd:review` 2026-09-07, [ISS-63](../../ISSUES.md)) — доменна функція `getLifecycleState` (T9) існує й протестована, але жоден use-case її не викликає; card ніколи не переходить у `in_use`, тож §7 KPI «медіанний час створена → заповнена → ведеться» не має джерела даних. Відкрите архітектурне питання — яка саме дія має тригерити перехід (перший підтверджений запис? створення блоку-метрики? інше)? **Позначка: наступний етап.** — owner: Андрій, due: перед першим виміром §7 KPI
- [ ] **AC-15: стан «колізія назви при трансфері» недосяжний з UI цієї фічі** (`/sdd:review` 2026-09-07) — `CardBack.tsx` рендерить стан і кнопку «Зберегти», але нічого їх не тригерить: сам трансфер ініціюється з фічі `structure` (AC-14 «`structure` AC-12»), яка ще не готова. Компонент лишається мертвим кодом, доки `structure` не підключить `POST /cards/{cardId}/metric-blocks/transfer`. **Позначка: узгодити разом з wiring `structure`.** — owner: Андрій, due: коли `structure` реалізує ініціацію трансферу
