...
Обраний підхід — Approach A з idea-brief.md §7/§13 («Одне поле, одна схема»): користувач кидає прямий запис в один канал (чат з агентом; формат — будь-який, що приймає платформа Claude, за D-54), агент сам пропонує, до якої картки й блоку-метрики він відноситься, користувач підтверджує. **Застереження:** у idea-brief.md цей підхід позначений як «запропоновано, не підтверджено» — кроки 9–11 (RICE, здійсненність, формальне підтвердження) протоколу `sdlc-interview` не пройдені (§11–§13). Ця специфікація рухається з ним як з робочою гіпотезою; формальне підтвердження — відкрите питання §8.
...
- [ ] Підхід A досі «запропоновано, не підтверджено» в idea-brief (RICE/Feasibility — TBD). Default now: рухаємось як з робочою гіпотезою, формальне підтвердження не блокує розробку. — owner: Андрій, due: до `/sdd:tasks life-area-card`
- [ ] Максимальна кількість карток? Default now: без жорсткого ліміту, лише попередження агента при наближенні до практичної стелі. — owner: Андрій, due: перед `/sdd:tasks life-area-card`
- [ ] Чи фіксуємо правило «похідні числа (прогрес, частка) не зберігаємо, лише перераховуємо з сирих подій» (Engineer, design-review блок 8)? Default now: не вирішено. — owner: Андрій, due: перед `/sdd:data-model life-area-card`
