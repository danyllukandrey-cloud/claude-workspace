...
### Flow: US-14 — Видалити картку, якою більше не користуюсь

```mermaid
flowchart TD
    A[SCR-02/SCR-03 Картка] -->|меню «...» → «Видалити»| B[SCR-06 Підтвердження архівації]
    B -->|скасовує| A
    B -->|підтверджує, AC-16| C[SCR-01 Колода карток — без цієї картки]
```

Видалення — завжди з підтвердженням; м'яка архівація, картка зникає з колоди й будь-якої розкладки Структури, лишається технічно відновлюваною.
