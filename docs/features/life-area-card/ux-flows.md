...
> ⚠️ **SCR-01→SCR-02 Entry/Exit застаріли 2026-09-14 ([D-122](../../DECISIONS.md#d-122)).** "Тап картки → SCR-02" описував окремий екран деталей (`CardDetailScreen`, ISS-55) — прибраний. Передня картка колоди в SCR-01 ОДРАЗУ показує SCR-02 (чи SCR-03, якщо перегорнута) — нема окремого кроку "відкрити". "Назад → SCR-01" (з SCR-02) теж прибрано — нема куди повертатись, картка вже в колоді. Purpose-колонка й самі стани (row 18 вище: "один компонент, що перевертається") лишаються чинними без змін — це якраз те, що завжди й малось на увазі.
