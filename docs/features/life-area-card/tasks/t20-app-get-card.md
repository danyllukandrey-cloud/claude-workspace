...
[ISS-34](../../../ISSUES.md) лишається відкритою — не виправлено тут (пре-існуюча прогалина T6, `computeProgress` кидає виняток для `targetCount=null`+`isOngoing=false`, хоча `data-model.md` документує це як валідний стан).
