...
**Оновлено 2026-09-06 ([ISS-52](../../../ISSUES.md)/[ISS-53](../../../ISSUES.md)):** екран входу (`App.tsx`/`LoginScreen.tsx`) і npm-скрипт `dev:server` для реального запуску сервера додані окремо, поза цим task-файлом (не product-AC `life-area-card` — app-shell-рівень, ADR-0006 §Додаток). Обидві нові змінні середовища (`JWT_SECRET`, `GOOGLE_CLIENT_ID`) і третя (`VITE_GOOGLE_CLIENT_ID`) уже додані користувачем у `.env`; сервер перевірено — реально піднімається (`npm run dev:server`).
