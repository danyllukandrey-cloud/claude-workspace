...
- **Інструмент «об'єднати дві картки»** — у v1 користувач закриває одну й веде далі в іншій вручну; спеціальний інструмент об'єднання розглядається пізніше, якщо практика покаже потребу (рішення цієї сесії, 2026-08-23).
...
**When** the user closes one card and continues tracking in the other
**Then** the system records the closure as a history event, with no dedicated merge action offered
...
- [ ] Щільність поля розкладки (запас вільних клітинок чи клітинки з половинним кроком, design-review Блок 4)? Default now: не вирішено. — owner: Андрій, due: перед `/sdd:design structure`
- [ ] Як спроєктувати форми так, щоб декларація Структури й Опис картки **структурно не могли перетнутись** (завдання з design-review Блок 2, не вирішене)? Default now: розділені текстові поля з різними підказками, без технічного захисту від змістового дублювання. — owner: Андрій, due: перед `/sdd:design structure`
