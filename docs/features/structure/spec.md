...
- [ ] Перенесення метрики закритої картки в іншу (AC-12, [D-65](../../DECISIONS.md#d-65)) — `life-area-card` spec.md ще не має acceptance criterion на прийом перенесеної метрики з чужої картки; чи метрика зберігає власну історію записів, чи зливається з існуючою метрикою цільової картки — не визначено. — owner: Андрій, due: перед `/sdd:data-model structure` чи наступною ітерацією `life-area-card` (що раніше)
- [ ] Реальне видалення картки з колоди карток ([D-66](../../DECISIONS.md#d-66)) — у Структурі AC-12 закриває напрямок лише в межах розкладки (м'який статус); `life-area-card` не має власного стану «видалено»/«архівовано» взагалі. Три варіанти (порожня картка / з метриками повністю / з перенесенням метрик) і форма показу аналітики щодо закритих напрямків — не визначено. — owner: Андрій, due: окрема ітерація `life-area-card`
