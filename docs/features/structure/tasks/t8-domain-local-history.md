...
[sad.md §5](../sad.md#5-building-block-view) — `history.ts`: локальна модель для офлайн-кешу; самі події пише репозиторій історії на бекенді (T10), не ця задача.
...
Це лише локальна модель клієнта — фактичний запис відбувається в базі бекенда (T10, T12, T13).
