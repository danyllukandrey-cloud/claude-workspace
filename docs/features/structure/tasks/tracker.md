...
**T28 додано 2026-09-12** ([D-115](../../../DECISIONS.md#d-115), закриває [ISS-105](../../../ISSUES.md)) — рев'ю 2026-09-11 знайшло дірку: AC-15 обіцяв подію 'renamed' у Літописі, місце в схемі й типи були, виклику не було. Юніт-тести зелені (мокнутий `Db`). **Round-trip проти реальної Neon перевірено 2026-09-13** — новий `update-card.integration.test.ts` (2 тести: подія записується з новим ім'ям; той самий виклик з незміненим ім'ям НЕ пише подію) — обидва зелені.

**T9/T12 виправлено 2026-09-12** ([D-117](../../../DECISIONS.md#d-117), закриває [ISS-101](../../../ISSUES.md)) — `insertLayoutPosition` (T9) писав `position_updated_at` за дефолтом колонки (`now()`), через що перше ж реальне переміщення картки (T12) могло тихо програти `resolvePositionConflict` через розбіжність годинників клієнт/БД. Виправлено на сентинел-час (епоха); новий номер задачі не заводили — це виправлення вже наявної реалізації, не нова функціональність. Юніт-тест зелений. **Round-trip проти реальної Neon перевірено 2026-09-13** — новий тест у `move-card.integration.test.ts`: щойно створена через `insertLayoutPosition` позиція одразу переміщується (`positionUpdatedAt: new Date()`), не програючи `resolvePositionConflict` — зелений.
