...
```mermaid
flowchart TD
    A[SCR-02 Схема: «Конфігурація»] -->|обирає один із п'яти плоских режимів| B{Картки вже розкладені під іншим способом?}
    B -->|ні, перший вибір| C[SCR-02 Схема: спосіб застосовано, board]
    B -->|так, зміна способу| D[SCR-02 Схема: картки скинуто в базовий порядок]
    D -->|перетягує картки заново під нову схему| C
```

Користувач відкриває «Конфігурацію» на екрані Схеми й обирає спосіб розкладки. Якщо це перший вибір (картки ще ніде не стояли) — спосіб просто застосовується (AC-11), і користувач повертається на board-вигляд Схеми. Якщо картки вже були розкладені під іншим способом — застосунок питає підтвердження (`ConfirmDialog`), після підтвердження всі вони скидаються вниз екрана в базовому порядку, і користувач розкладає їх заново під нову схему клітинок (AC-11b).
