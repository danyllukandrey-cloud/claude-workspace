...
> Це джерело правди про **клон**: де лежить, як оновлювати, як під'єднати.
> Про **ролі** двох репозиторіїв курсу і де шукати рішення — `PROJECT-STATE §13.11`.

**Крок 1 — клон на комп'ютері Андрія.** ✅ Зроблено: тека
`Desktop/agentic-engineering-course-public`, поруч із `claude-workspace`.
...
⚠️ **Своя робота в цей клон не комітиться** — репозиторій чужий, `push` неможливий,
а копії файлів розходяться. Рішення `D-13`, історія — `PROJECT-STATE §13.11`.
