...
> картка — «Навчання» (`life-area-card`, [DECISIONS D-22](./DECISIONS.md)): прогрес книг,
> курсів і сертифікацій відносно цілі. Мапу згенеровано `survey` під час foundation-сесії
> (greenfield-режим, бо репозиторій був порожній); домен першої картки на момент генерації
> мапи ще не був обраний, тому нижче лишились технічні описи, актуальні незалежно від домену.
...
C4Container
    ...
    Container(pwa, "ПЛАН (PWA Frontend)", "React + TypeScript + Vite", "Показує картки, рахує прогрес навчання, керує UI")
    ContainerDb(storage, "Локальне сховище", "Browser localStorage", "Зберігає введені дані користувача (позиції навчання, прогрес) в межах одного браузера/пристрою")
    System_Ext(importApi, "Сервіс імпорту прогресу", "зовнішній API (майбутнє, не обов'язково)", "У майбутньому підтягує прогрес із Goodreads/Coursera тощо")
    Rel(user, pwa, "Відкриває як застосунок, вводить дані в картки")
    Rel(pwa, storage, "Читає/пише стан через шар абстракції storage")
    Rel(pwa, importApi, "[МАЙБУТНЄ] запит прогресу з зовнішнього трекера", "HTTPS")
```
...
| card:life-area-card | `plan/app/src/cards/life-area-card/` | UI + логіка розрахунку | `plan/app/src/app/main.tsx` (реєстрація картки) | Введення позицій навчання (книга/курс/сертифікація) із ціллю в кількості й часі → розрахунок прогресу (факт/ціль × 100%) → відображення на звороті картки |
...
- Нова "картка" (здоров'я/кар'єра/…) → `plan/app/src/cards/<slug>/`, за зразком `life-area-card` (перший прецедент, буде створено при `implement` фічі)
- Новий спільний UI-примітив → `plan/app/src/shared/ui/`, за зразком `CardShell`
- Майбутня інтеграція з зовнішнім сервісом імпорту прогресу (Goodreads/Coursera тощо) → окремий модуль `plan/app/src/shared/progress-import/` + ADR про спосіб виклику (напряму з браузера чи через невеликий сервер-проксі, залежно від CORS-обмежень постачальника даних)

## Обмеження та відомий технічний борг

- Дані не синхронізуються між пристроями (свідомий компроміс MVP, зафіксовано в ADR-0003) — новий пристрій = порожня картка навчання
- Прогрес позначається вручну (немає ще підключення до зовнішнього API) — закладено місце (`shared/progress-import/`), але не реалізовано
