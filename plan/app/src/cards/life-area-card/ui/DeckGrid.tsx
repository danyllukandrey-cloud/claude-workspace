...
export function DeckGrid({ items, renderFront }: DeckGridProps): JSX.Element {
  ...
  return (
    ...
    //
    // justify-end (CH-13 follow-up, живе тестування, Андрій): без нього
    // вільний простір лишався б ПІД стрілками ‹›/пейджером (діти flex-col за
    // замовчуванням top-aligned), і відступ до кнопок "Створити картку"/
    // "Архів карток" (DeckScreen.tsx) ставав набагато більшим за gap-3/py-3
    // нав-меню внизу. justify-end притискає колоду (картка + пейджер) до низу
    // цього контейнера -- відступ до кнопок знову дорівнює тому самому gap-3.
    //
    // Без max-w-xs (CH-13 follow-up #2, живе тестування, Андрій зі
    // скріншотом зменшеного через Ctrl масштабу): "навіть якщо я зменшу
    // масштаб картки мають відформатовуватись на весь екран" -- max-w-xs
    // фіксував ширину колоди в 20rem НЕЗАЛЕЖНО від розміру вікна, тож на
    // збільшеному (через менший масштаб) видимому просторі картка лишалась
    // тим самим малим квадратом серед порожнього поля. Ширина тепер керована
    // самою карткою (h-full/aspect-square нижче), не цим контейнером.
    <div className="mx-auto flex w-full min-h-0 flex-1 flex-col items-center justify-end gap-3">
      {/* CH-13 (docs/features/life-area-card/changes.md, живе тестування,
          Андрій): "картки в колоді мали квадратну форму" -- замінює D-121's
          висото-керований розмір (h-[85%], коментар вище лишається для
          історії рішення) на aspect-square. Розмір і далі керується ВИСОТОЮ
          (h-full -- та сама ідея, що D-121, 85% через flex-1 предка), ширина
          обчислюється з неї через aspect-square; max-w-full -- запобіжник на
          вузькому/високому вікні, де рівний квадрат інакше виліз би за межі
          вширшки. */}
