...
export async function transferMetricBlock(db: Db, input: TransferMetricBlockInput): Promise<MetricBlockRecord> {
  ...
  // Review 2026-09-07 (backend hardening, T50, "трансфер у архівовану
  // цільову картку приймається"): картка-ПРИЗНАЧЕННЯ архівована -- findCardById
  // сам не фільтрує за status (він валідний, "мій", просто закритий), тому
  // без цієї перевірки блок міг опинитись у картці, недосяжній із Колоди.
  // Контракт (openapi.yaml) не документує окремого коду для цього випадку --
  // той самий 'card.not_found', що вже покриває "не знайдено"/"чужа" (один
  // код на всі причини, ISS-30) -- НЕ вигадуємо новий код без контракту (той
  // самий клас помилки, що ISS-82/ISS-83). Картка-ДЖЕРЕЛО навпаки МАЄ право
  // бути архівованою (AC-14: трансфер відбувається САМЕ тому, що вона
  // закривається) -- перевірка стосується лише targetCard.
  if (!block || !sourceCard || !targetCard || targetCard.status === 'archived') {
    // Один код на всі причини (ISS-30) -- контракт не розрізняє "блок не
    // існує", "блок чужий" і "картка-призначення чужа/не існує/архівована".
