...
import {
  ...
  reportUserRequestedIssue,
  DeveloperReportValidationError,
...
describe('reportAgentDetectedError -- AC-20', () => {
  it('produces a valid developer_report payload from only the agent\'s own error text, no user text required', () => {
    const report = reportAgentDetectedError({
      id: 'report-1',
  ...
  it('defaults userId to null when the error was not tied to a specific user session', () => {
    const report = reportAgentDetectedError({ id: 'report-2', errorSummary: 'Unhandled DB error' });
  ...
  it('rejects an empty/whitespace-only error summary -- description is NOT NULL in the schema', () => {
    expect(() => reportAgentDetectedError({ id: 'report-5', errorSummary: '   ' })).toThrow(
      DeveloperReportValidationError,
    );
  });
});
...
describe('reportUserRequestedIssue -- AC-20b', () => {
  it('produces a valid developer_report payload that preserves the user\'s description verbatim', () => {
    ...
    const report = reportUserRequestedIssue({ id: 'report-6', userId: 'user-1', userDescription: verbatim });
  ...
  it('does not trim or otherwise rewrite the user\'s wording', () => {
    ...
    const report = reportUserRequestedIssue({ id: 'report-7', userId: 'user-1', userDescription: withSpacing });
    expect(report.description).toBe(withSpacing);
  });

  it('rejects an empty/whitespace-only user description -- description is NOT NULL in the schema', () => {
    expect(() =>
      reportUserRequestedIssue({ id: 'report-8', userId: 'user-1', userDescription: '   ' }),
    ).toThrow(DeveloperReportValidationError);
  });
});
...
describe('developer_report -- shared invariants across both trigger types', () => {
  it('sets trigger_type correctly and distinctly for each origin', () => {
    const agentReport = reportAgentDetectedError({ id: 'report-9', errorSummary: 'boom' });
    const userReport = reportUserRequestedIssue({ id: 'report-10', userId: 'user-2', userDescription: 'boom' });
